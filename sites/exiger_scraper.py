...
def collect_data_from_exiger() -> list[dict]:
    ...
    '''
    response = requests.get(url='https://www.exiger.com/careers/', headers=DEFAULT_HEADERS)
    soup = BeautifulSoup(response.text, 'lxml')

    soup_data = soup.find_all('section', class_='level-0')
    print(soup_data)
...
print(collect_data_from_exiger())
# @update_peviitor_api
