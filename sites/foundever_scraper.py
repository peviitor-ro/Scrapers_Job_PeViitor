...
def collect_data_from_foundever():
    ...
    lst_with_data = []
    for sd in soup_data:
        title_section = sd.find('span', class_='jobTitle hidden-phone')
        location_section = sd.find('td', class_='colLocation hidden-phone')
        link = 'https://jobs.foundever.com' + title_section.find('a')['href']
        title = title_section.find('a').text.strip()
        cities = [
            city.strip() for city in location_section.text.replace(', RO', '').split(',') if city.strip()
        ]
