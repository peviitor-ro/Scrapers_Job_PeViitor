...
# Link to company career page -> https://rolandberger-search-api.e-spirit.cloud/v1/prepared_search/JoinJobs/execute/?language=en&query=*&facet.filter.country=%22Romania%22
#
#
#
#
...
JOBS_URL = 'https://rolandberger-search-api.e-spirit.cloud/v1/prepared_search/JoinJobs/execute/?language=en&query=*&facet.filter.country=%22Romania%22'


def return_lst_dict(title: str, link: str, cities: list[str]) -> dict:
    '''
...
def collect_data_from_rolandberger():
    ...
    for job in get_jobs():
        title = job.get('title', [''])[0].strip()
        link = job.get('positionProfile___webAddress', [job.get('link', '')])[0]
        cities = [
            translate_city(city.strip())
            for city in job.get('location', [])
            if city and city.strip()
        ]

        if title and link and cities:
            lst_with_data.append(return_lst_dict(title=title, link=link, cities=cities))
